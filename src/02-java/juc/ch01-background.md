...
简单来讲，基于时间片的运行方式带来了原子性问题，不同缓存之间的访问速度的差异带来了可见性问题，编译器的优化带来了有序性问题，也就是说计算机物理结构决定了运行在操作系统之上的高级编程语言，要想实现并发编程的设计目标，就必须要考虑这三个基本问题。换句话来说，这三个基本问题就成了并发编程的线程安全的三要素了。
...
<summary>实现 Runnable 接口</summary>
...
<summary>实现 Callable 接口</summary>
