...
1. 精简指令集 复杂指令集
2. 手机 cpu 多使用精简指令集， 个人电脑 cpu 多使用复杂指令集；但最近几年，随着科技的发展，手机 cpu 性能越来越强，也开始进军个人电脑领域；
3. jdk 常见的安装包有
   1. oracle 推荐使用；官网提供了多个版本，但整体支持三个 os 平台，每个平台两种 cpu 架构；
      1. 三个 os 平台： linux、 macos、 windows；
      2. 两种 cpu 架构： arm（arrch） 和 x64；
   2. openjdk
   3. Azul 公司的 Azul Zulu 版本；
   4. Amazon 公司的 Amazon Corretto；
   5. Eclipse 的 Eclipse Temurin；
