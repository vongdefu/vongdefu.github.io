...
![技术派实战项目](./javase/image/bdfa153fc82310f9ab862a1b3db0d0d7.png)

后面又做了微服务项目 [pmhub](https://javabetter.cn/zhishixingqiu/pmhub.html)，接触到了 Spring Cloud、Nacos、Sentinel、Seata、SkyWalking 等相关技术栈。

![pmhub](./javase/image/1719412227941-391d1ca0-e312-4e81-a958-2eff29dbecd7.png)
...
![MYDB](./javase/image/javase-20241223085416.png)
...
![三分恶面渣逆袭：Java语言特点](./javase/image/javase-2.png)
...
![三分恶面渣逆袭：JDK、JRE、JVM关系](./javase/image/javase-3.png)
...
![三分恶面渣逆袭：Java程序执行过程](./javase/image/javase-4.png)
...
![三分恶面渣逆袭：编译与解释](./javase/image/javase-5.png)
...
![二哥的 Java 进阶之路：Java数据类型](./javase/image/nine-01.png)
...
![三分恶面渣逆袭：Java自动类型转换方向](./javase/image/javase-7.png)
...
![三分恶面渣逆袭:装箱和拆箱](./javase/image/javase-8.png)
...
![break 、continue 、return](./javase/image/javase-9.png)
...
![kaito：浮点数](./javase/image/javase-20240321112428.png)
...
![kaito：25.125](./javase/image/javase-20240321113232.png)
...
![三分恶面渣逆袭：面向对象和面向过程的区别](./javase/image/javase-10.png)
...
![技术派实战项目源码截图](./javase/image/javase-20241111172421.png)
...
![二哥的 Java 进阶之路：封装继承多态](./javase/image/javase-20240330115129.png)
...
![截图来自博客园的小牛呼噜噜：虚拟方法表](./javase/image/javase-20241126104207.png)
...
![二哥的 Java 进阶之路](./javase/image/21-01.png)
...
![里氏替换原则由芭芭拉·利斯科夫提出，照片摄于2010年](./javase/image/javase-20240321103119.png)
...
![访问修饰符和可见性](./javase/image/javase-12.png)
...
![二哥的 Java 进阶之路：接口不能定义构造方法](./javase/image/javase-20240512090855.png)
...
- 修饰类时， 表明类不能被继承；
- 修饰方法时，表明方法不能被Override（即重写或覆盖）；
- 修饰变量时，表明变量的值不能被改变；
  - 修饰引用类型的变量时，表明这个引用类型的变量不能再指向别的对象；至于引用类型指向的对象的内容，是可以随着对象的变化而变化的；
...
- 相同点： 三个都属于Java的关键字；
- 不同点：
  - final 用来修饰类、方法、变量等；修饰类时，类不可被改变；修饰方法时，方法不能被Override（重写或覆盖）；修饰变量时，变量的值不能被改变（引用类型不能改变指向的内容）；
  - finally是异常处理时使用的关键字，常和try块一起搭配使用。无论try是否抛出异常，finally块都会被执行，常用来资源的关闭；
  - finalize是超类Object的一个方法，用于垃圾回收时的对象的回收；这个方法是由jvm中负责垃圾回收的线程自动调用的，日常开发中很少使用到；
...
![二哥的 Java 进阶之路，String的equals()源码](./javase/image/javase-20240425093626.png)
...
值传递。即便是引用对象，传递的也是这个引用指向的对象的地址。例如，把一个对象作为参数传递给一个方法，方法实际上接收到的参数是这个对象的引用的地址。

#### 引用类型的变量有什么特点？

> 从jvm的角度回答。

引用类型在内存中同时具有两块存储区域，一块是句柄，里面存放着引用类型所引用的对象的位置；另一块则是这个对应在内存中的存储区域，里面存放着对象的属性和方法；操作数栈和对象池；

引用类型的变量在jvm进行垃圾回收时，在经过二次标记之后依然存在引用，则可以逃脱回收。
...
![三分恶面渣逆袭：浅拷贝和深拷贝示意图](./javase/image/javase-15.png)
...
![三分恶面渣逆袭：Java创建对象的四种方式](./javase/image/javase-16.png)
...
![三分恶面渣逆袭：堆与常量池中的String](./javase/image/javase-17.png)
...
![三分恶面渣逆袭：jdk1.8之前的字符串拼接](./javase/image/javase-18.png)
...
![二哥的 Java 进阶之路：integer 源码](./javase/image/javase-20240323080956.png)
...
![二哥的 Java 进阶之路：调整缓存池大小](./javase/image/javase-20240323082802.png)
...
![parseInt示意图](./javase/image/javase-20.png)
...
![三分恶面渣逆袭：Object类的方法](./javase/image/javase-21.png)
...
`public String toString()`：返回对象的字符串表示。默认实现返回类名@哈希码的十六进制表示，但通常会被重写以返回更有意义的信息。比如，我们常常会返回json对象，以便于日志的记录。
...
![](./javase/image/javase-20240313085055.png)
...
![三分恶面渣逆袭：Java异常体系](./javase/image/javase-22.png)
...
![三分恶面渣逆袭：异常处理](./javase/image/javase-23.png)
...
![二哥的Java 进阶之路：catch 和 finally 处理异常](./javase/image/javase-20241008095737.png)
...
![二哥的 Java 进阶之路](./javase/image/shangtou-01.png)
...
![Java IO流用到装饰器模式](./javase/image/javase-25.png)
...
![二哥的 Java 进阶之路：IO 分类](./javase/image/javase-20240404103618.png)
...
![三分恶面渣逆袭：BIO](./javase/image/javase-27.png)
...
![三分恶面渣逆袭：NIO](./javase/image/javase-28.png)

缓冲区 Buffer 也能极大提升一次 IO 操作的效率。

![三分恶面渣逆袭：NIO完整示意图](./javase/image/javase-29.png)

#### 简单说下 AIO？

AIO 是 Java 7 引入的，放在 java.nio.channels 包下，提供了 AsynchronousFileChannel、AsynchronousSocketChannel 等异步 Channel。

它引入了异步通道的概念，使得 I/O 操作可以异步进行。这意味着线**程发起一个读写操作后不必等待其完成，可以立即进行其他任务，并且当读写操作真正完成时，线程会被异步地通知**。
...
序列化（Serialization）是指将实体对象转换为字节流的过程，以便能够将该对象保存到文件、数据库，或者进行网络传输。

反序列化（Deserialization）就是将字节流转换回对象的过程，以便构建原始对象。

序列化的过程，相当于把人类能够理解的变成语言转换成机器能够理解的二进制语言的过程；反序列化的过程则是相反。

序列化机制中还隐藏了一个概念，那就是序列化的数据模板。例如，把一个对象序列化成字节流时，对象的类就是数据模板。这个数据模板的作用是，保证序列化与反序列化过程中的数据的一致性。

![三分恶面渣逆袭：序列化和反序列化](./javase/image/javase-30.png)
...
可以使用`transient`关键字修饰不想序列化的变量。（但静态变量默认不会被序列化，因此 transient 对静态变量没有影响。）
...
![Java常见序列化方式](./javase/image/javase-31.png)
...
![二哥的Java 进阶之路：一个简单的 socket 通信](./javase/image/socket-20230330192826.png)
...
![博客园struggler：经典的 RPC](./javase/image/javase-20241128182231.png)
...
![泛型类、泛型接口、泛型方法](./javase/image/javase-32.png)
...
也就是说，在运行的时候，虚拟机会把泛型转化成具体的类型，之后的过程就是没有泛型的。
...
- 保持兼容性。为了向下兼容，因为 JDK5 之前是没有泛型的，为了让 JVM 保持向下兼容，就出了类型擦除这个策略。
- 运行期的优化。泛型在编译时基本上就可以确定要使用的类型是哪一个了，就没有必要在运行时，在进行查找了，直接在编译期就确定好数据的类型，省得运行期还要查找数据的类型。
...
![Override注解](./javase/image/javase-34.png)
...
![Autowired注解](./javase/image/javase-35.png)
...
![三分恶面渣逆袭：Java反射相关类](./javase/image/javase-36.png)
...
![三分恶面渣逆袭：JDK1.8主要新特性](./javase/image/javase-37.png)
...
![Java Stream流](./javase/image/javase-38.png)
