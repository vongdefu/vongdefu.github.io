...
5. 在 `传输数据`的过程，要考虑以下几个问题
   ...
   5. 加了滑动窗口之后，解决了一方无法一下子处理过多数据的问题，但是有网络带宽可能并没有压榨完，也就是说滑动窗口可以是动态的，那怎么办？`流量控制`，让双方协商，主动告诉对方能够处理的缓冲区的大小，让另一方酌情发送！
6. 在 `请求销毁通道`的过程中，也会出现一些问题：
   1. 客户端通信完成之后，服务器端能否主动关闭通道？不能（客户端有可能没有接收到某个数据分组，会要求服务器端重新发送数据分组）。那怎么办？只能让客户端发送`请求销毁通道`的信息给服务器端。同理，服务器端通信完成之后，能不能让客户端主动关闭？也不能（客户端有可能还没有接收完成），也只能通过服务器端通知客户端关闭的方式关闭通道。【这里说不准，因为有时候客户端和服务器端的角色有可能会发生互换，并且每一端都可以服务多个其它端，比如客户端请求多个服务端，一个服务器端响应多个客户端】
...
> 被动开启： 连接不能自动开启传输通道，要被动开启，这样节省资源。
